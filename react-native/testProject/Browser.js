...
What I need to do is to update state
the state I want to update is

I think I need the page to be interactable for the user. So that means it still is web based.

So instructions get injected into the app.

So I need very similar code as before but I don't need to detect across tabs or events, I just need to know what url the user is on. I can get that via the window objects.
...
// `;
// const injectedJavaScript = `
//       window.ReactNativeWebView.postMessage(document.URL);
//       true; // note: this is required, or you'll sometimes get silent failures
// `;

/*
So you can inject html selectors into the same document.

So the next thing to do is to have an orchestrator function taking a config for a user

and orchestrating the injections into the document.

processCommands
===========
next big unknown is:

how to interact with other apps
...
const injectedJavaScript = `
      window.ReactNativeWebView.postMessage(document.querySelector("div.nav-link-wrapper a").innerText);
      true; // note: this is required, or you'll sometimes get silent failures
`;
